...
Разработчики работают совместно над всем проектом — и backend, и frontend. Старое разделение зон отменено.

- `API_DOCS.md` — контракт между фронтом и бэком, обновляется при любых изменениях API
